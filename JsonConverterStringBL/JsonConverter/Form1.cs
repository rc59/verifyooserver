        ...
        private  void getDataFromDB()
        {
            if (!string.IsNullOrEmpty(_fileName))
            {
                ...
                try
                {
                    if (_name.Length > 0)
                    {
                        IMongoQuery query = Query<ModelShapes>.EQ(c => c.Name, _name);                        

                        ModelShapes shapes = listMongo.FindOne(query);
                        ...
                        foreach (ModelShape obj in shapes.ExpShapeList)
                        {
                            ...
                            for (int idxStroke = 0; idxStroke < obj.Strokes.Count; idxStroke++)
                            {
                                ...
                                for (int idxEvent = 0; idxEvent < obj.Strokes[idxStroke].ListEvents.Count; idxEvent++)
                                {
                                    ...
                                    tempEvent.AccelerometerX = obj.Strokes[idxStroke].ListEvents[idxEvent].AngleX;
                                    tempEvent.AccelerometerY = obj.Strokes[idxStroke].ListEvents[idxEvent].AngleY;
                                    tempEvent.AccelerometerZ = obj.Strokes[idxStroke].ListEvents[idxEvent].AngleZ;
                                    ...
                                    tempStroke.Xdpi = shapes.Xdpi;
                                    tempStroke.Ydpi = shapes.Ydpi;
                                }

                                tempStroke.Length = obj.Strokes[idxStroke].Length;
                                tempGesture.Instruction = obj.Instruction;
                                tempGesture.ListStrokes.add(tempStroke);
                            }

                            tempTemplate.Name = shapes.Name;
                            tempTemplate.ModelName = shapes.ModelName;
                    ...
                    else
                    {
                        while (!isFinished)
                        {
                            shapesList = listMongo.FindAll().SetSortOrder(SortBy.Descending("Created")).SetLimit(limit).SetSkip(skip);
